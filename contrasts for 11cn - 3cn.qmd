...
True or false: The effect of stereotyping is significant (use α=0.05). `r torf(TRUE)`
...
How large is the R-square? `r fitb(.294, number = T, tolerance = .01)`
...
What's the p-value for the overall test of model fit? `r fitb(.117, number = T, tolerance = .01)`
...
What is the regression slope for scholastic aptitude? $b = `r fitb(.47, number = T, tolerance = .01)`$.
...
What is the adjusted group mean for the group that received the Negative Stereotype manipulation? `r fitb(6.359, number = T, tolerance = .01)`
...
How much of the variance in Math do SA and STEREO explain? `r fitb(.421, number = T, tolerance = .01)`

 

Controlled for SA, how much of the remaining variance in Math does STEREO explain? Use the formula mentioned in the lecture slides to calculate the partial $\eta^2$: `r fitb(.152, number = T, tolerance = .01)`
