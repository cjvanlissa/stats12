...
## Coefficienten {.smaller .flexbox .vcenter}
...
## Coefficienten
Waarom is Work_hours eerst niet significant, en wel wanneer we Gender_roles toevoegen?
...
## Twee simpele regression voorbeelden
Twee aparte simpele regressies, $\hat{Y}_i = a + b_1*X_1$
...
werkplot1 <- ggplot(invdat, aes(x=Work_hours, y=Involvement))+
  ...
  geom_abline(intercept = m_work$coefficients[1], slope = m_work$coefficients[2], colour = "black", size = 1)
...
genderplot1 <- ggplot(invdat, aes(x=Gender_role, y=Involvement))+
  ...
  scale_y_continuous(limits = c(0,50), breaks=seq(0, 50, by = 5))+
  geom_abline(intercept = m_gender$coefficients[1], slope = m_gender$coefficients[2], colour = "black", size = 1)
...
## Multiple regression voorbeeld
Een multiple regression, $\hat{Y}_i = a + b_1*X_1 + b_2*X_2$
...
## Multiple regression voorbeeld 2
Het lijkt alsof de lijnen wel de trend volgen, maar buiten de data vallen. 

Dit is niet het geval: Het effect van Work_hours is gecontrolleerd voor het effect van Gender_role, en vice versa. 

[$\color{red}{\text{Het wordt duidelijker als je dit als 3D plot visualiseert...}}$](https://plot.ly/~c.j.vanlissa/3.embed)
...
## Centreren
> $a$: De $\color{blue}{\text{intercept}}$, voorspelde waarde wanneer **alle**  X-en gelijk zijn aan 0

Dit is wat onhandig, omdat er bijna niemand is die 0 uur werkt, en helemaal niemand die 0 scoort op de Likert-schaal voor Gender_roles (1-7)

## Centreren
Daarom verschuiven we het nulpunt:

$\text{Center}(Y_i) = Y_i - \bar{Y} = \text{observaties - gemiddelde}$

```{r, fig.width=6, fig.asp=3/4, echo=FALSE, warning=FALSE, message=FALSE}
scale_dat <- data.frame(Scaled = factor(c(rep(0, 100), rep(1, 100)), labels = c("Standaard", "Gecentreerd")), Gender_role = c(invdat$Gender_role, scale(invdat$Gender_role, scale = FALSE)))
...
## Multiple regression na centreren
Na het centreren zien de aparte plots van multiple regression er zo uit:
...
## Centreren
Door te centreren kan je een betekenisvol nulpunt kiezen voor de predictoren, bijvoorbeeld het gemiddelde.
...
```{r out.height = "350px"}
knitr::include_graphics("./Materials/Interactie.png", dpi = NA)
```
