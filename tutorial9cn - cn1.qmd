...
But a crucial question is: which means are different from each other? We can use contrasts to answer this question.

Suppose we are interested in the average difference in feelings between a guide who sings, and a guide who tells jokes.

Formulate H0 and H1 for this specific comparison, then check your answer.
...
$H_0 =$ `r fitb(0, num = T)` $*\mu_{rush} +$
`r fitb(0, num = T)` $*\mu_{story +}$
`r fitb(0, num = T)` $*\mu_{insult +}$
`r fitb(1, num = T)` $*\mu_{joke +}$
`r fitb(-1, num = T)` $*\mu_{sing +}$
 
We can evaluate the contrast using the `\CONTRAST` command in SPSS syntax.

First, we will make a basic ANOVA syntax.

Analyze > Compare means > One-way ANOVA.

Select feeling in the dependent list, and choose behavior as the factor.
Paste the syntax. It should look like this:

```
ONEWAY feeling BY behavior
  /ES=OVERALL
  /MISSING ANALYSIS
  /CRITERIA=CILEVEL(0.95).
```

Next, we need to add the command for the contrast.

Consult the lecture slides for an example of the syntax for running contrasts. SPSS requires you to, basically, just list the numbers with which you want to multiply the means in the order of the value labels (see Variable view).

So if you want to compare rushing with telling stories, and this is the order of the labels:

rushing |	telling stories |	insulting	| making jokes | singing
--------|-----------------|-----------|--------------|---------
1 |	-1 |	0 |	0 |	0


Then this would be the syntax for the contrast:
...
Now, modify the syntax for comparing the joking and singing groups.

Run the syntax when you've adapted it.
 

First, consult the table labeled ANOVA. What does the F-test test?

`r longmcq(c(
answer = "The overall significance effect of behavior on feeling.",
"The  significance of the contrast.",
"The significance of the variance of feeling.",
"Between-groups homoscedasticity (equality of variances)."
)[sample.int(4)])`


Consult the output of Contrasts Tests.

What is the (absolute) value of the contrast? `r fitb(.6365, num = T, tol = .01)`

How can we interpret this contrast value?

`r longmcq(c(
answer = "It is the mean difference between the two groups.",
"It is the test statistic for the contrast.",
"It is the difference between 1 and -1, standardized by the between-groups variance.",
"It is an F ratio."
)[sample.int(4)])`

Report the results of the statistical test for the contrast and draw a conclusion.

`r hide("Answer")`

On average, there is no significant difference between the group that had a singing guide (M =  6.4611) and the group that had a guide making jokes (M = 6.2620) regarding the feelings of the participants about the guide, t(45) = -0.482, p = .632.
`r unhide()`
...
How many comparisons do we have to make? `r fitb(3, num = T)`

What is the syntax for each of these comparisons?
...
H0: 3∗\murush - 2∗\mustory + 3∗\muinsult-2∗\mujoke-2∗\musing = 0
...
0.5*\mu1+ 0*\mu2 + 0.5*\mu3- 0.5*\mu4 - 0.5*\mu5 = 0
