...
We consider two estimation methods:
...
```{r}
tab_bic <- data.frame(Factors = 1:3, matrix(c(81.122, 43, 238.332,	54, 46.279,	33), byrow = T, nrow = 3))
...
* Factors are allowed to be correlated with each other
...
* In social science, many factors are probably correlated!
    + Oblique rotation is considered to be default by many

## Oblique Rotation ctd.

**Note:** factor loadings and item/factor correlations are now distinct!
...
* Multiply standardized item scores with factor loadings for that component
* Divide resulting variable by Eigenvalue of that component

<!-- as.matrix(scale(data)) %*% t(pracma::pinv(loadings))) -->


# How to analyse these scores: CTT

* Or: Sumscore =  <span style="color:#FF0000">1</span> \*X1 \+  <span style="color:#FF0000">1</span> \*X2 \+  <span style="color:#FF0000">1</span> \*X3….\+  <span style="color:#FF0000">1</span> \*X14  (Unweighted sum\!)
* <span style="color:#FF0000"></span>  <span style="color:#FF0000"> All variables have the same weight\!\!:</span>
  * __Online behavior __ =  <span style="color:#FF0000">w</span> .X1 \+  <span style="color:#FF0000">w</span>  <span style="color:#FF0000">.</span> X2 \+ … \+  <span style="color:#FF0000">w</span> .X14
    * We have to calculate the sumscore  _separately_  _ _  _for_  _ _  _every_  _ _  _dimension_
    * That’s why, when using classical test theory (and calculating an unweighted sumscore), we don’t need to know what items belong to which dimension
    * This is different in PCA: we give an item a different weight for every dimension

# PCA: components

* Not calculating a sum score, but trying to find patterns in the data
  * Based on these patterns, we find components
  * Components = dimensions of the construct
  * To which dimensions does an item belong?  higher weight for this dimension\!
* A  <span style="color:#FF0000">component</span>  is thus representative for a subconstruct (a dimension)
  * e.g., NEO big five  the 5 personality traits

# PCA: component scores & component loadings

  * Component \#1 =  <span style="color:#EA2D00">w</span>  <span style="color:#EA2D00">1</span> .XZ1 \+  <span style="color:#EA2D00">w</span>  <span style="color:#EA2D00">2</span>  <span style="color:#EA2D00">.</span> XZ2 \+ … \+  <span style="color:#EA2D00">w</span>  <span style="color:#EA2D00">14</span> .XZ14 		(XZ  Standardized variables\!)
  * _	(For example: _  <span style="color:#EA2D00"> _0_ </span>  <span style="color:#EA2D00"> _._ </span>  _XZ \+ _  <span style="color:#EA2D00"> _0.5_ </span>  <span style="color:#EA2D00"> _._ </span>  _XZ \+ … \+ _  <span style="color:#EA2D00"> _0.5_ </span>  _._  _XZ14)_
  * Component \#2 =  <span style="color:#EA2D00">w2</span>  <span style="color:#EA2D00">1</span> .XZ1 \+  <span style="color:#EA2D00">w2</span>  <span style="color:#EA2D00">2</span>  <span style="color:#EA2D00">.</span> XZ2 \+ … \+  <span style="color:#EA2D00">w2</span>  <span style="color:#EA2D00">14</span> .XZ14
  * _	(For example: _  <span style="color:#EA2D00"> _0.8_ </span>  <span style="color:#EA2D00"> _._ </span>  _XZ \+ _  <span style="color:#EA2D00"> _0.1_ </span>  <span style="color:#EA2D00"> _._ </span>  _XZ \+ … \+ _  <span style="color:#EA2D00"> _0.1_ </span>  _._  _XZ14)_
* PCA calculates for every item a weight w1, w2, w3 , w4 , …, w14 for every component (we call this a component loading)
  * The loading is a correlation between component and item
  * Indicates which dimension an item measures: if item 1 belongs to component/dimension1, then this item will get a high weight for this component and a very low weight for the second component/dimension

  * Component\#1 =  <span style="color:#EA2D00">w</span>  <span style="color:#EA2D00">1</span> .XZ1 \+  <span style="color:#EA2D00">w</span>  <span style="color:#EA2D00">2</span>  <span style="color:#EA2D00">.</span> XZ2 \+ … \+  <span style="color:#EA2D00">w</span>  <span style="color:#EA2D00">14</span> .XZ14 		(XZ  Standardized variables\!)
  * _	(For example: _  <span style="color:#EA2D00"> _0_ </span>  <span style="color:#EA2D00"> _._ </span>  _XZ \+ _  <span style="color:#EA2D00"> _0.5_ </span>  <span style="color:#EA2D00"> _._ </span>  _XZ \+ … \+ _  <span style="color:#EA2D00"> _0.5_ </span>  _._  _XZ14)_
  * Component\#2 =  <span style="color:#EA2D00">w2</span>  <span style="color:#EA2D00">1</span> .XZ1 \+  <span style="color:#EA2D00">w2</span>  <span style="color:#EA2D00">2</span>  <span style="color:#EA2D00">.</span> XZ2 \+ … \+  <span style="color:#EA2D00">w2</span>  <span style="color:#EA2D00">14</span> .XZ14
  * _	(For example: _  <span style="color:#EA2D00"> _0.8_ </span>  <span style="color:#EA2D00"> _._ </span>  _XZ \+ _  <span style="color:#EA2D00"> _0.1_ </span>  <span style="color:#EA2D00"> _._ </span>  _XZ \+ … \+ _  <span style="color:#EA2D00"> _0.1_ </span>  _._  _XZ14)_
  * We can calculate component scores for every respondent: a score on C1 and score on C2
  * _The _  _answers_  _ on _  _the_  _ items (XZ1, XZ2, etc..) are _  _weighted_  _ _  _based_  _ on _  _the_  _ component _  _loadings_  _. _  _By_  _ _  _doing_  _ _  _so_  _, items _  _that_  _ _  _measure_  _ _  _dimension_  _ 1 (component 1) _  _contribute_  _ _  _the_  _ most _  _to_  _ _  _the_  _ _  _calculation_  _ of _  _the_  _ component score _  _for_  _ _  _the_  _ first _  _dimension_  _. _

## How does this work? What does SPSS do??

  * Component\#1 =  <span style="color:#FF0000">w</span>  <span style="color:#FF0000">1</span> .XZ1 \+  <span style="color:#FF0000">w</span>  <span style="color:#FF0000">2</span> .XZ2 \+ … \+  <span style="color:#FF0000">w</span>  <span style="color:#FF0000">14</span> .XZ14
  * _	(For example: _  <span style="color:#FF0000"> _0_ </span>  _._  _XZ1 \+ _  <span style="color:#FF0000"> _0.5_ </span>  _._  _XZ2 \+ … \+ _  <span style="color:#FF0000"> _0.5_ </span>  <span style="color:#FF0000"> _._ </span>  _XZ14)_
* Finding the  <span style="color:#FF0000">weighted</span>  sum scores with weights  <span style="color:#FF0000">w</span>  <span style="color:#FF0000">1</span>  <span style="color:#FF0000">, w</span>  <span style="color:#FF0000">2</span>  <span style="color:#FF0000">, … w</span>  <span style="color:#FF0000">14</span> , such that we can explain as much variance as possible
* Variance = information\!
* This score is then “principal component\#1”
