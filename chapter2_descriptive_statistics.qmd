...
While some descriptive statistics are *calculated in the same way* as inferential statistics (which are first introduced in later chapters),
...
```{r fig-medians, fig.cap="Location of the median in symmetrical and asymmetrical distributions."}
...
```{r figsquare, fig.cap="When squared, large values are more influential than small values."}
library(ggplot2)
...
```{r fig-ecwagenmakers, fig.cap="An interpretation of De Groot’s empirical cycle, by Wagenmakers, Dutilh, & Sarafoglou, 2018. CC-BY: Artwork by Viktor Beekman, concept by Eric-Jan Wagenmakers"}
