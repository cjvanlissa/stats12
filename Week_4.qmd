# Philosophy of Science {#sec-testing}
...
In order for causality to be established, three conditions must be met, and only one of these conditions is covariation of these two variables, which implies that when X changes, Y must change too. However, two more conditions must be met: *Temporal precedence*, which means that X happens before changes occur in Y; and *Non-spuriousness*, referring to the existence of no third variable Z that can explain the changes in both X and Y. These criteria summarize what observational patterns must look like if a causal story is true (Oppewal, 2010). A randomized controlled experiment is a situation where it is possible to enforce these three conditions.
